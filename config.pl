...
my @external = qw(iflport ifport omp_lib netcdf ifcore grib_api iso_c_binding mpi iso_fortran_env); 
...
my @mpi_modules = qw(silam_partitioning silam_mpi);
my @no_mpi_modules = qw(silam_no_partitioning silam_no_mpi);
...
SOURCEFILE: foreach my $file_path(@sourcefiles) {
	...
	if (defined($fhdot) && !$in_type_def && ($line =~ /^\s*private\s*$/i || $line =~ /^\s*private\s*\!/i) && $modname) {
