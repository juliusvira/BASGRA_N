subroutine BASGRA( PARAMS, MATRIX_WEATHER, &
                   ...
                   NDAYS, NYEARS, soilcn_option, NOUT, y)
!-------------------------------------------------------------------------------
...
integer                   :: day, doy, i, NDAYS, NOUT, year, nyears
...
!real :: soilc(num_c_pools), dsoilc(num_c_pools), norg
...
if (soilcn_model == 'yasso') then
   call init_yasso('parameters/yasso_parameters', 'initialisation/yasso_init', yasso_inst)
...
do cycyear = 1, nyears
   do day = 1, NDAYS
      ...
      if (day == 1 .and. soilcn_model == 'basgra') then
         ...
         print '(A, 4G12.2)', 'TOTC, NORG, NMINf, NMIN ', get_totc(yasso_inst), get_norg(yasso_inst), nMineralisation, NMIN
         print *, 'Cumulative litterfall, gpp and respiration:', cumlittc, cumphot, cumsoilr
...
! Extra variables
  ...
  y(total_day,101) = GSTUB+GRT+GST+GLV ! NPP
...
! State equations plants
  ...
  ! Mineral N same in basgra and yasso
  ...
  NMIN    = max(0.,NMIN)
  print *, 'NMIN end of step:', NMIN, NMIN/TCNUPT
  print *, 'Nfluxes', Ndep, Nfert, Nmineralisation, Nfixation, NSHmobsoil, Nupt, Nleaching, Nemission
  print *, 'litterfall:', DLV, DSTUB, DRT
  print *, 'inhib gpp:', phot*fngrowth
